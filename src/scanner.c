...
static void multilineComment() {
  while (peek() != ']' && !isAtEnd()) {
    if (peek() == '\n') scanner.line++;
    if (match('#') && peek() == '[') multilineComment();
    ...
    case 's':
      if (scanner.current - scanner.start > 1) {
        switch (scanner.start[1]) {
          ...
          case 't': return checkKeyword(2, 1, "d", TOKEN_STD);
...
static Token string(char closing) {
  while (peek() != closing && !isAtEnd()) {
    if (peek() == '\n') scanner.line++;
    advance();
  }
